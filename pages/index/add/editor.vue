			...
			<view class='wrapper'>
				...
				<view class="editor-wrapper">
					<editor id="editor" class="ql-container" placeholder="开始输入..." showImgSize showImgToolbar showImgResize
					 @statuschange="onStatusChange" :read-only="readOnly" @blur="onEditorBlur" @ready="onEditorReady">
					</editor>
				</view>

        <u-button type="success" @click="saveEditor">保存</u-button>
			</view>
		</view>
	</view>
</template>
...
<script>
import { memorandumAdd } from '../../../api/index/index';
export default {
	data() {
		return {
			readOnly: false,
    ...
    // 保存
    saveEditor(){
      memorandumAdd({message:this.contentMessage,shortTitle:this.shortTitle}).then(res=>{
        console.log(res);
      })
    },
    // editor
    onEditorBlur(e){
      console.log(e);
      this.contentMessage = e.detail.html
      this.shortTitle = e.detail.text
    },
		readOnlyChange() {
